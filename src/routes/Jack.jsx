...
import { Box, Drawer, Flex, Input, Skeleton } from "@chakra-ui/react";
...
const Jack = () => {
  ...
  const onSell = async (e) => {
    ...
    try {
      ...
      toaster.create({
        title: err.status,
        description: (err.customer && "please select customer") || err.detail,
        type: "error",
        duration: 5000
      })
  ...
  const handleCus = (e) => {
    ...
    if (
      cus.first_name && cus.last_name && cus.email && cus.phone && cus.address === ""
    ) {
      toaster.create({
        title: "please enter customer values",
        type: "error"
      })
      return;
    }
  ...
  return (
    <div className="flex flex-col sm:flex-row h-dvh relative font-roboto w-full">
      ...
      <div className="sm:w-2/3 z-50 w-full h-dvh">
              ...
              <Dialog.Positioner>
                <Dialog.Content className="dark:bg-opacity-30">
                  ...
                  <Dialog.Footer>
                    ...
                    <Button onClick={handleCus} className="bg-[#f7d518] text-gray-900 px-6 font-bold">CREATE</Button>
                  </Dialog.Footer>
                </Dialog.Content>
              </Dialog.Positioner>
            </Portal>
        ...
        {loading ? <div className="w-full my-4 grid p-2 grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
      ...
      <div className="sm:w-1/3 invisible sm:visible w-0 relative h-dvh place-items-center">
        ...
        <div className="w-full flex flex-col shadow-sm rounded-md">
          {cart.map(item => <div key={item.id} className="bg-white dark:bg-opacity-10 shadow-sm items-center m-2 rounded-xl flex p-2">
            <img src={`https://grandypos.duckdns.org${item?.images?.[0]?.image}`} className="rounded-sm size-16" />
                  ...
                  <Dialog.Content className="dark:bg-opacity-30">
                    ...
                    <Dialog.Body >
                      <p className="  font-roboto flex">
                        You're about to create order of total price <p className=" ml-1 font-bold">{total.toLocaleString()} Tshs</p>
                      </p>
                    </Dialog.Body>
                    <Dialog.Footer>
                      <Dialog.ActionTrigger asChild>
                      ...
                      <Button onClick={onSell} className="bg-[#f7d518] text-gray-900 px-6 font-bold">CREATE</Button>
                    </Dialog.Footer>
                  </Dialog.Content>
                </Dialog.Positioner>
              </Portal>
